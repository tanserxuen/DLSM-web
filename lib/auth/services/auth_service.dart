...
class AuthService extends RiverpodService {
  ...
  ResetPasswordTokenStateNotifier get _resetPasswordTokenStateNotifier => ref.read(resetPasswordTokenStateProvider.notifier);
  ...
  ResetPasswordTokenState get _resetPasswordTokenState => ref.read(resetPasswordTokenStateProvider);
  ...
  Future<void> signIn(SignInRequestDTO dto) async {
    if (_authTokensState.accessToken != null) throw Exception('Already authenticated. Access token is not null');

    Response response = await _dio.post('auth/signin', data: dto.toJson());
    AuthTokensResponseDTO tokens = AuthTokensResponseDTO.fromJson(response.data);
  ...
  Future<void> renewTokens() async {
    AuthTokensState state = _authTokensState;
    if (state.refreshToken == null) throw Exception('Not authenticated. Refresh token is null');
    
    Response response = await _dio.post(
      'auth/refresh',
      options: Options(headers: {'Authorization': 'Bearer ${state.refreshToken}'})
    );
    AuthTokensResponseDTO tokens = AuthTokensResponseDTO.fromJson(response.data);
    await _refreshTokenHiveService.setRefreshToken(tokens.refreshToken);
    _authTokensStateNotifier.setTokens(tokens.accessToken, tokens.refreshToken);
  ...
  Future<void> requestResetPassword(RequestResetPasswordRequestDTO dto) async {
    ...
    Response response = await _dio.post('auth/request-reset-password', data: dto.toJson());
    RequestResetPasswordResponseDTO requestResetPasswordResponseDto = RequestResetPasswordResponseDTO.fromJson(response.data);
    _resetPasswordTokenStateNotifier.setToken(requestResetPasswordResponseDto.resetPasswordToken);
  }
  ...
  Future<void> resetPassword(ResetPasswordRequestDTO dto) async {
    ...
    if (resetPasswordToken == null) throw Exception('Reset password token is null');

    await _dio.post(
      'auth/reset-password', 
      data: dto.toJson(),
      options: Options(headers: {'Authorization': 'Bearer $resetPasswordToken'})
    );
